...
from db.models import Expediente

SCRIPT_NAME = "load_expedientes"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    session = SessionLocal()
    data = leer_json_local("expedientes_transformado.json")
    logging.info("Cargando expedientes...")
    for item in data:
        session.add(Expediente(**item))  # adaptación al modelo real
    session.commit()
    session.close()
