...
from db.models import Anuncio

SCRIPT_NAME = "load_anuncios"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    session = SessionLocal()
    data = leer_json_local("anuncios_transformado.json")
    logging.info("Cargando anuncios...")
    for item in data:
        session.add(Anuncio(**item))  # adaptación al modelo real
    session.commit()
    session.close()
