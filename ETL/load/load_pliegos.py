...
from db.models import Pliego

SCRIPT_NAME = "load_pliegos"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    session = SessionLocal()
    data = leer_json_local("pliegos_transformado.json")
    logging.info("Cargando pliegos...")
    for item in data:
        session.add(Pliego(**item))  # adaptación al modelo real
    session.commit()
    session.close()
