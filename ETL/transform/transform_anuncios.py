...
SCRIPT_NAME = "transform_anuncios"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    data = leer_json_local("anuncios.json")
    logging.info("Transformando anuncios...")
    transformed = data  # placeholder para lógica real
    guardar_json_local(transformed, "anuncios_transformado.json")
