...
SCRIPT_NAME = "transform_adjudicatarios"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    data = leer_json_local("adjudicatarios.json")
    logging.info("Transformando adjudicatarios...")
    transformed = data  # placeholder para lógica real
    guardar_json_local(transformed, "adjudicatarios_transformado.json")
