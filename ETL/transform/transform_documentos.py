...
SCRIPT_NAME = "transform_documentos"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    data = leer_json_local("documentos.json")
    logging.info("Transformando documentos...")
    transformed = data  # placeholder para lógica real
    guardar_json_local(transformed, "documentos_transformado.json")
