...
SCRIPT_NAME = "transform_organos"
logging.basicConfig(level=logging.INFO, format="%(asctime)s - " + SCRIPT_NAME + " - %(message)s")

def main():
    data = leer_json_local("organos.json")
    logging.info("Transformando organos...")
    transformed = data  # placeholder para lógica real
    guardar_json_local(transformed, "organos_transformado.json")
